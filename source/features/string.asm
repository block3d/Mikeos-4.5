...
; ------------------------------------------------------------------
; os_string_copy_add_astr -- add '*' in front of string
; IN/OUT: SI = source, DI = destination (programmer ensure sufficient room)

os_string_copy_add_astr:
	pusha

	mov al, 02Ah			; replace 1st char with '*'
	mov [di], al
...
; ------------------------------------------------------------------
; os_string_copy_rem_astr -- remove '*' in front of string
; IN/OUT: SI = source, DI = destination (programmer ensure sufficient room)

os_string_copy_rem_astr:
	pusha

.more:
	mov al, [si + 1]			; Transfer contents (at least one byte terminator)
	mov [di], al
	inc si
	inc di
	cmp byte al, 0			; If source string is empty, quit out
